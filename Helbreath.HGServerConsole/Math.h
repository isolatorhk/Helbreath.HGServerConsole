...
inline uint32 GetNibble(uint32 var, uint8 index)
{
	index *= 4;
	return (var & (0xF << index)) >> index;
}

inline void SetNibble(uint32 &var, uint8 index, uint8 val)
{
	index *= 4;
	var &= UINT_MAX - (0xF << index);
	var |= val << index;	
}

inline uint32 GetBit(uint32 var, uint8 index)
{
	return (var & (0x1 << index)) >> index;
}
