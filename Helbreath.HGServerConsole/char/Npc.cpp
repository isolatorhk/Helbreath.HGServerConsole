...
CNpc::CNpc(char * pName5, int NpcH):Unit(UNITTYPE_NPC)
{
	int i;
...
int CNpc::GetPoisonResistRatio() {
	return 0;
}
int CNpc::GetFrostResistRatio() {
	if (m_cAttribute == ATTRIBUTE_WATER) return 100; 
	return m_cResistMagic-(m_cResistMagic/3); 
}
			...
			(abs(sX - dX) <= m_iAttackRange) && (abs(sY - dY) <= m_iAttackRange)) {
				...
				case 87: // CT
					...
					case 79: 
						if (target)
						{
							if (!target->IsDead() &&  checkResistingIceSuccess(m_cDir, target, m_iMagicHitRatio) == FALSE) 
							{
								if (target->m_cMagicEffectStatus[ MAGICTYPE_ICE ] == 0) 
								{
									...
									RegisterDelayEvent(DELAYEVENTTYPE_MAGICRELEASE, MAGICTYPE_ICE, dwTime + (5*1000), 
